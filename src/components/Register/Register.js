...
function Register(props) {
  ...
  const { showPopup, isLogged } = React.useContext(AppContext);
      ...
      <div className='register__content'>
        ...
        <form name='register' className='form' onSubmit={handleSubmit}>
          ...
          <fieldset className='form__fieldset'>
            ...
            <input type='text' id='name' name='name' className='form__input' aria-label='Имя' placeholder='Введите имя'  value={values['name']} onChange={handleChange} disabled={isInputDisabled} required/>
          ...
          <button type='submit' className={`form__submit-btn ${isValid ? 'form__submit-btn_active' : '' }`} aria-label='Зарегистрироваться' disabled={isInputDisabled} >{isLoading ? 'Создание профиля...' : 'Зарегистрироваться'}</button>
